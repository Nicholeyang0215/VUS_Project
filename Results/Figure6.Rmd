...
# find a VUS carrier with high positive predictive value. 
```{r  }

dt.VUS = val3[which(brca1$Myriad_panel=="VUS" | brca1$Myriad_panel=="VUS - Suspect Del/ Likely Pathogenic"),]
dt.brca = dt.VUS[which(dt.VUS$GenTestTable.Gene.1.gt == "BRCA1" | dt.VUS$GenTestTable.Gene.1.gt == "BRCA2"),]

score = as.numeric(as.character(dt.brca$BRCA1or2))
logit_scores = log(score/(1-score))

dt.ppv = point_estimate1*df1(logit_scores)/(point_estimate1*df1(logit_scores) + (1-point_estimate1)*df2(logit_scores))

dt.ppv[order(dt.ppv, decreasing = T)]

order.ppv = dt.brca[order(dt.ppv, decreasing = T), ]
large.ppv = order.ppv[1:2,]
...
y1 = means2[85:1285]    ## means of PPV
y2 = lower_q2[85:1285]  ## lower bond of ppv 
y3 = upper_q2[85:1285]  ## upper bond of ppv 
...
lik <- function(pi) sum(log(pi*df1_1(x3)+(1-pi)*((df2_1(x3) - theta*df1_1(x3))/(1-theta))))
...
point_estimate = rep(NA, 2000)
n_carriers = rep(NA, 2000)
n_VUS = rep(NA, 2000)
  
for (i in c(1:2000))
{
  
  indx = sample(1:1995, size = 1995, replace = TRUE)
  new_brca = brca1[indx,]

  pos = new_brca[which(new_brca$Myriad_panel=="Positive"),]
  pos_brca = pos$BRCA1or2[which(pos$GenTestTable.Gene.1.gt == "BRCA1" | pos$GenTestTable.Gene.1.gt =="BRCA2")]

  VUS = new_brca[which(new_brca$Myriad_panel=="VUS" | new_brca$Myriad_panel=="VUS - Suspect Del/ Likely Pathogenic"),]
  VUS_brca = VUS$BRCA1or2[which(VUS$GenTestTable.Gene.1.gt == "BRCA1" | VUS$GenTestTable.Gene.1.gt == "BRCA2")]

  neg = new_brca[which(new_brca$Myriad_panel == "Negative"),]
  neg_brca = neg$BRCA1or2
  ...
  df1_1 <- approxfun(density(x1, bw = "sj", from = -15, to =11))
  df2_1 <- approxfun(density(x2, bw = "sj", from = -15, to =11))
